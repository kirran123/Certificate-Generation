...
export default function UserDashboard() {
  ...
  const groupedReceived = receivedCerts.reduce((acc, cert) => {
    const bid = cert.batchId || 'Individual Certificates';

    const s = receivedSearch.toLowerCase();
    const batchMatches = bid.toLowerCase().includes(s);
    const certMatches = !s ||
      cert.name?.toLowerCase().includes(s) ||
      cert.email?.toLowerCase().includes(s) ||
      cert.certificateId?.toLowerCase().includes(s);
                    ...
                    <div className="border-t border-[var(--border-subtle)] overflow-x-auto">
                      <table className="w-full text-left">
                        <thead><tr className="border-b border-[var(--border-subtle)] bg-[var(--border-subtle)]">
                          {['Recipient', 'Certificate ID', 'Created'].map(h => <th key={h} className="px-6 py-3 text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">{h}</th>)}
                        </tr></thead>
