...
// Delete an entire batch of certificates
router.delete('/delete-batch/:batchId', protect, async (req, res) => {
  ...
  try {
    ...
    await Certificate.deleteMany(filter);
    
    // If it's an automation-run batch, also delete the snapshot in FormAutomation if needed
    // (Optional: we can just leave snapshots or delete them if they match the batchId)
    await FormAutomation.deleteMany({ ...filter, active: false });

    res.json({ message: `Successfully deleted batch "${batchId}" and its ${certs.length} certificates.` });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
